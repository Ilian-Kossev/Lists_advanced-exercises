...
def strike_at_index(collection: list, strike_index: int, radius: int):
    ...
    if is_index_valid(collection, left_strike_index) and \
        ...
        for _ in range(left_strike_index, right_strike_index + 1):
...
numbs_str_list = input().split()
numbers = [int(num) for num in numbs_str_list]
...
numbers_str_sting = [str(num) for num in numbers]
print('|'.join(numbers_str_sting))
